...
primitivo** — consulte a lista atual em `docs/design/DESIGN.md`; copiar o bloco BEM de um
primitivo passou a ser defeito (`PROBLEMS.md` `P-034`). O primitivo é dono da identidade
(cor, raio, fonte, estado); o tamanho e o layout ficam na sua classe BEM, no mesmo elemento.
`docs/design/tema/_componentes.scss` é mapa histórico, não fonte de cópia. Reutilize o padrão do
consumidor único quando não houver primitivo, sem transformá-lo em catálogo novo. BEM em português.
Tailwind para layout/espaçamento/tipografia. `[appTooltip]` em vez do `title` nativo do browser.
Primeiro um corte visual representativo — não um formulário HTML genérico coberto de tokens por cima.
