 ...
 * agente, multiplicada por `sinal` (`1` aplica, `-1` desfaz — ver `reverterBonusConsumoFragmento`,
  ...
  switch (opcao.tipo) {
    case 'DEFESA':
      return agente.derivados.defesa !== undefined
        ? { ...agente, derivados: { ...agente.derivados, defesa: agente.derivados.defesa + sinal * opcao.valor } }
        : agente;
    case 'DANO_CORPO':
      return agente.derivados.danoCorpoACorpo
        ? {
            ...
            derivados: {
              ...agente.derivados,
              danoCorpoACorpo: somarDanoFixo(agente.derivados.danoCorpoACorpo, sinal * opcao.valor),
            },
          }
        : agente;
    case 'TESTE': {
...
/**
 * Desfaz o bônus "Consumido" de um fragmento sobre o agente (m3-64, correção) — `sinal: -1` de
 * `aplicarBonusConsumoFragmento`, reusando o mesmo switch (proibições #26/#27). Usada ao **remover**
 * um registro de `dados.fragmentosConsumidos`: o consumo original não é mais "irreversível" — é
 * apenas trabalhoso de reverter à mão, e a ficha faz esse trabalho.
 */
export function reverterBonusConsumoFragmento(
  agente: EfeitoConsumoFragmentoDto,
  opcao: OpcaoBonusConsumoFragmentoDto,
  atributoEscolhido: keyof FichaAtributosDto | null,
): EfeitoConsumoFragmentoDto {
  return aplicarBonusConsumoFragmento(agente, opcao, atributoEscolhido, -1);
}
