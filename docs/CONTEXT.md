...
> Atualizado após cada sessão de implementação. Última atualização: 2026-07-06 (m1-17 — singleton em memória `EstadoAbasCalculadoraService` que preserva o formulário das 5 abas `agente`/`dt`/`novo-agente`/`patente`/`descanso` ao trocar de aba, sem sobreviver a F5; preset da aba `agente` passa a Nível 0 e atributos 1/1/1/1/1 — sem tocar em regra de jogo). Sessão anterior no mesmo dia: m1-16 — refinamento do sistema de tema em runtime (slot de cor custom salvo, inversão por incompatibilidade de base, paleta expandida).
...
| M1 | Calculadora com paridade | **concluído no código** (`m1-01` a `m1-17`, incluindo os refinamentos pós-paridade: mobile `m1-15`, tema em runtime `m1-16` e singleton de estado das abas `m1-17`; resta a task de refinamento `m1-18`). Restam 2 passos operacionais de plataforma: publicar a Cloudflare Pages e arquivar o repo antigo no GitHub (ver `docs/PARIDADE-M1.md`) |
...
estado das abas — 17 tasks). Resta **uma** task de refinamento no **backlog**, ainda não iniciada:
