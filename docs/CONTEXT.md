...
> Atualizado após cada sessão de implementação. Última atualização: 2026-07-08 (**m3-03 —
...
+ enum `TipoFichaEnum`) e **m3-03** (backend do CRUD da ficha de jogador — módulo `ficha` com a matriz
de permissões §14 e a validação do documento contra `shared/regras`; verificado ao vivo contra o
Postgres) **concluídos** (specs em `docs/specs/done/`).

**Próxima task: `m3-04`** — concessão/revogação de acesso de visualização (`usuario_ficha_acesso`): o
mestre/dono revela uma ficha a outro membro da campanha, que passa a poder vê-la (mas nunca editá-la).
A leitura da concessão já é consumida pela permissão de visualização da m3-03; a m3-04 adiciona
`concederAcesso`/`revogarAcesso`. Depois: **m3-05** (tempo real — emissão de eventos WS pós-mutação da
ficha, já cabeada no service).
