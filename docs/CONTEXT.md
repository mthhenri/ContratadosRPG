...
> Atualizado após cada sessão de implementação. Última atualização: 2026-07-08 (**m1-20 — modo
...
> só apresentação/estado — regra 100% no motor, proibição #26): **alternador Comprar/Vender** no topo
> (reusa o chip `.compras-categoria` — padrão de tab ativo em `--accent` + alvo de toque mobile já
> vêm dele, proibição #29). O modo roteia leituras/escritas para um **carrinho de venda separado**
> (`carrinhoVenda`/`amplificadoresVenda`) via helpers `lerCarrinho`/`definirCarrinho` — **o carrinho
> de compra e sua persistência m1-11 ficam 100% intactos** (Comprar mantém a tela atual; alternar não
> mistura as listas). Card "Venda" (só no modo Vender) com **seletor de taxa** (Normal 50 / Check-in
> 75 / Fora de patente 25), **valor de venda dos itens**, **grade de fragmentos** (Módulo V→I ×
> Potencializador/Construtor, contadores −/+, rótulo de tipo por célula — empilha limpo no mobile sem
> cabeçalho de colunas, subtotal por linha) e **Total de Venda** = itens (na taxa) + fragmentos em
> **stat box de destaque accent**. "Dinheiro Restante" some no modo Vender (sem sentido lá);
> Exportar/Importar (persistência do carrinho de compra) só aparecem em Comprar. **Limpar (m1-19)**
> passou a zerar também o modo (volta a Comprar), o carrinho de venda, a taxa e os contadores de
> fragmentos. **+4 testes** de página (Vitest, frontend **130/130**): taxa aplicada pelo motor (50/75/25),
> fragmentos pela tabela + total combinado, independência dos dois carrinhos ao alternar, e Limpar
> zerando o modo Venda. Fora de escopo (viraram **nota na Ajuda m1-12**, não trava de cálculo):
> "equipamento inicial só vende ao atingir Operador", "item inutilizável não tem valor" e o Módulo ∅
> (negociado com o Mestre); forja/redução de módulo também fora. **Ajuste de budget:** o modo Vender
> levou o CSS da `compras.page` (a página mais pesada) de ~9,9 kB para 11,14 kB, então o
> `anyComponentStyle` do `angular.json` subiu de **10→12 kB (warning) / 12→14 kB (error)** — mesmo
> precedente do bump de 565 kB do bundle inicial, aprovado pelo autor; build volta a ficar sem
> warning. `lint`/`test`/`build` (562,92 kB inicial, dentro do budget, **sem warning**) verdes;
> **verificado por render** (Playwright/Chromium) em 1200px e 360px: matemática confere na tela
> (item $250 × 50% = $125; Módulo V Potencializador ×2 = $1.000; Módulo I Construtor ×1 = $15.000;
> total $16.125), **zero scroll horizontal** nas duas larguras. Spec `m1-20` em `done/`. **M1 fecha
> com 20 tasks.** Sessão anterior no mesmo dia: **refino mobile da
...
delete; fecha a matriz §14; verificado ao vivo contra o Postgres; backend 76/76) **concluídos** (specs
em `docs/specs/done/`).

**Próxima task: `m3-05`** — tempo real (WebSocket): emissão de eventos de negócio (`ficha:alterada`,
`ficha:criada`) **broadcast-only** após cada mutação bem-sucedida da ficha (§9), consumindo a mesma
verificação de permissão da service (proibição #28, sem duplicar regra no gateway). A escrita continua
entrando só por REST; o gateway nunca recebe mutação.
...
- **m1-20 — modo Vender na aba Compras** (2026-07-08): task complementar do M1 (após a m1-19), 100%
  ...
  alternador Comprar/Vender (reusa `.compras-categoria`), carrinho de venda **separado** roteado por
  modo (o de compra e a persistência m1-11 intactos), card "Venda" com seletor de taxa, grade de
  fragmentos (V→I × Potencializador/Construtor, subtotal por linha) e Total de Venda em stat box de
  destaque; "Dinheiro Restante" some no modo Vender, Exportar/Importar só em Comprar, e Limpar (m1-19)
  zera o modo/venda/taxa/fragmentos; **+4 testes** de página (frontend 130/130). Restrições fora de
  escopo viraram **nota na Ajuda** (equipamento inicial só vende ao atingir Operador, item inutilizável
  sem valor, Módulo ∅ negociado com o Mestre), não trava de cálculo; forja/redução de módulo também
  fora. Budget `anyComponentStyle` do `angular.json` subiu p/ **12/14 kB** (a página mais pesada cresceu
  com o 2º modo — aprovado pelo autor, precedente do bump de 565 kB). `lint`/`test`/`build` (562,92 kB,
  sem warning) verdes; **verificado por render** (Playwright, 1200/360px): matemática confere na tela,
