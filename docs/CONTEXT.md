...
> `membro:entrou` na sala `campanha:<id>`. Payloads: `ficha:alterada`/`ficha:criada` reusam
> `FichaAlteradaDto`/`FichaCriadaDto`; **`membro:entrou` ganhou DTO novo no shared**
> (`CampanhaMembroEntrouDto { campanhaId, usuarioId }`, notificação da sala, distinta da
