...
| Primitivos de UI (código, não cópia) | [`frontend/src/app/shared/ui/`](../../frontend/src/app/shared/ui/) — `app-botao`/`app-campo` (`ui-01`), `app-modal`/`Notificacao` sobre `<dialog>` nativo (`ui-02`), `app-cartao`/`app-stat`/`app-chip`/`app-abas`+`app-aba`+`AbaPainel`/`app-step-input` (`ui-03`, `stepper/` promovido de `modules/simulacao`), `app-estado-vazio`/`app-esqueleto` (`ui-14`, estados de lista vazio/carregando) | pôr um botão, campo, modal, notificação, cartão, caixa de estatística, selo, barra de abas, stepper, estado vazio ou esqueleto de carregamento na tela |
