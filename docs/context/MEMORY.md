...
| Componentes reutilizáveis | `frontend/src/app/shared/` — `layout/`, `icone/`, `bandeja-dados/`, `historico-rolagens-sidebar/`, `calculadora-flutuante/`, `montador-rolagem/` (teclado de tokens da "Rolagem rápida", `ui-35` — teclas próprias em BEM tipo `calculadora-flutuante`, não `app-botao`; só escreve na `FormControl` que já existe, nenhuma regra de dados aqui), `tempo-real/`, `tooltip/`, `overflow-fade/`, `hold-repeat/`, `marca/`, `receber-dano/` (dialog "Receber dano", m7-17)… |
