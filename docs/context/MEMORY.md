...
| Primitivos de UI (código, não cópia) | [`frontend/src/app/shared/ui/`](../../frontend/src/app/shared/ui/) — `app-botao`/`app-campo` (`ui-01`), `app-modal`/`Notificacao` sobre `<dialog>` nativo (`ui-02`) | pôr um botão, campo, modal ou notificação na tela |
