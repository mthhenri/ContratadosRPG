...
| `.stat` | Caixa de estatística (rótulo + valor grande) | `vida` (`--vida`, fixo — não `--accent`), `energia` (`--energy`), `positivo` (`--positive`); `0` é valor real, enquanto `null`/`undefined`/texto vazio exibem traço em `--text-mute`; `[tamanho]` `compacto`/`padrao`/`hero` (destaque das simulações, `P-054`); `[statInfo]` projeta um botão de ajuda ao lado do rótulo; `[pulso]` incrementa para disparar um pulso de escala no valor (`P-054`) | **`<app-stat rotulo="…" [valor]="…">`** |
