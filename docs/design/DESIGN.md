...
| `.botao-icone` | Ação unitária sem rótulo visual | `mini` (16px, sem borda — ícone inline dentro de outro controle, ex. dadinho de um pill), `compacto` (26px) e `padrao` (32px); em mobile `compacto`/`padrao` passam ao alvo de toque de 44px (`mini` não, é sempre inline). Aceita `<button>` ou `<a>` (ui-28). Exige `aria-label` e `appTooltip`; foco e desabilitado são nativos (`:disabled` não afeta `<a>`) | **`<button app-botao-icone aria-label="…" appTooltip="…">`** ou **`<a app-botao-icone aria-label="…" appTooltip="…">`** |
