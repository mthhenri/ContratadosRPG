...
5. Oito fichas de jogador/agente: Matheus, Codex e os dois stubs possuem uma ficha diferente em
   cada campanha. Todas têm cores de identidade explícitas e distintas e JSONB compatível com o
   contrato atual.
...
- Rodar o seed duas vezes mantém exatamente 4 usuários ativos, 2 campanhas fixture, 8 vínculos
  fixture e 8 fichas fixture, sem duplicatas.
...
- `senhor.contratados` e `codex.dev` autenticam pela API real e veem uma campanha como mestre e a
  outra como jogador.
- As oito fichas abrem e são editáveis na aplicação; suas cores são distintas e respeitam
  `^#[0-9A-Fa-f]{6}$`.
