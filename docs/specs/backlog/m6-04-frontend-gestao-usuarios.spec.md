...
2. **`UsuarioService` (frontend)** ganha o transporte HTTP puro (DTOs do shared, extrai `dados`
   do `StandardResponse`, JWT via interceptor): `listarUsuarios`, `criarUsuario`,
   `alterarUsuario`, `alterarTipoUsuario`, `excluirUsuario` — mapeando às rotas da m6-03. Não
   redefine DTO (todos vêm do shared).
...
4. **Tela de gestão** (`paginas/gestao-usuarios/`, standalone, Signals, Reactive Forms):
   ...
   - Erros (login duplicado, "não pode rebaixar/excluir o último admin") vêm do backend e
     aparecem via `error-handler` (toast) — não reimplementar a regra no front.
5. **Estilos** `.scss`/BEM **só com tokens** do tema (proibição #29 — copiar blocos sancionados de
...
6. **Testes** (Vitest): `admin.guard.spec` (admin passa, não-admin redireciona, sem sessão vai
   ao login); `usuario.service.spec` (cada método atinge rota/verbo/corpo e mapeia `dados`);
   `gestao-usuarios.page.spec` (lista carrega; criar chama `criarUsuario`; trocar tipo chama
   `alterarTipoUsuario`; excluir exige confirmação → `excluirUsuario`; menu "Usuários" só para
   admin); `app.routes.spec` (`/usuarios` sem sessão → login; com sessão não-admin → `/painel`;
   com admin → resolve a tela). Adicionar `/usuario` ao proxy de dev se necessário (já existe da
   m2-11).

## Critérios de Aceite

- Só admin vê o item de menu "Usuários" e acessa `/usuarios`; não-admin é redirecionado.
- Admin lista, cria, altera, exclui e troca o tipo de usuários pela tela.
...
- Reset de senha de terceiros pelo admin (só se a m6-03 tiver entregue o endpoint).
