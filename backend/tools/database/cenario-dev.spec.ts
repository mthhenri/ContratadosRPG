...
describe('CENARIO_DEV', () => {
  it('mantém as quatro contas canônicas e preserva a senha do autor', () => {
    ...
    expect(CENARIO_DEV.usuarios.filter(({ alterarSenha }) => alterarSenha)).toHaveLength(3);
  });

  it('define as campanhas simétricas com dois mestres e seis jogadores', () => {
  ...
  it('não repete chaves de negócio do cenário', () => {
    expect(new Set(CENARIO_DEV.usuarios.map(({ chave }) => chave)).size).toBe(4);
    expect(new Set(CENARIO_DEV.usuarios.map(({ login }) => login)).size).toBe(4);
