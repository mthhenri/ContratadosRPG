 ...
 *
 * **Sem guarda de papel (m7-06).** A rota é a mesma para mestre e jogador — a casca
 * (`PainelEncontroShell`, `ui-39`) resolve o papel e monta `PainelEncontroMestre` ou
 * `PainelEncontroJogador`, como `CampanhaDetalheShell` faz na campanha. Quem barra é o backend, que
 * recorta o payload por usuário; um guard aqui só duplicaria a regra §14 e ainda obrigaria a manter
 * duas rotas para a mesma tela.
 *
...
export const encontroRoutes: Routes = [
  {
    path: '',
    loadComponent: () =>
      import('./paginas/painel/painel-shell.page').then((modulo) => modulo.PainelEncontroShell),
  },
  {
    path: ':encontroId',
    loadComponent: () =>
