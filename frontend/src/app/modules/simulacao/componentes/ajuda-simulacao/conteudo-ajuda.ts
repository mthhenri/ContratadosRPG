/**
 * Conteúdo de ajuda por aba da simulacao — equivalente ao `HELP_CONTENT` do site antigo
 * (`contratados-calculadora`), consumido pelo componente reutilizável `AjudaSimulacao`.
 *
...
/** Aba da simulacao que possui conteúdo de ajuda (casa com o `caminho` da rota). */
...
export const CONTEUDO_AJUDA: Readonly<Record<AbaAjuda, ConteudoAjuda>> = {
  ...
  compras: {
    ...
    passos: [
      ...
      'A aba Vendas (na barra da simulacao) monta um carrinho de venda separado (mesmo catálogo e modificações) e mostra quanto renderia — escolha a taxa (Normal 50%, Check-in 75% ou Fora de patente 25%). O bloco de Fragmentos soma módulos I–V por Potencializador / Construtor, e o Total de Venda combina itens + fragmentos.',
    ],
    nota: 'O carrinho de compra é salvo automaticamente neste navegador (o de venda não). Use Exportar / Importar para transferi-lo — o código é próprio desta simulacao e não é compatível com o do site antigo. Ressalvas de venda que a simulacao não modela: o equipamento inicial só pode ser vendido ao atingir Operador, um item inutilizável não tem valor, e o Módulo ∅ de fragmento é negociado com o Mestre (fora da tabela).',
  },
};
